    ...
    try {
      ...
      if (data.user) {
        const { error: profileError } = await supabase.from('profiles').insert({
          id: data.user.id,
          email: data.user.email, // Asumiendo que quieres guardar el email en el perfil
        ...
        if (profileError) {
          ...
          setLoading(false);
          return; // Detener la ejecución para mostrar el error de perfil
        }
      }
    ...
    <div className="container mx-auto p-4">
      ...
      <form onSubmit={handleSignIn} className="bg-white p-6 rounded shadow-md">
        ...
        <div className="flex items-center justify-between">
          <button
            type="submit"
            onClick={handleSignIn} // Se activa con el submit del formulario
