-- Script para agregar columnas de bloqueo de usuarios a la tabla profiles
...
-- 7. Opcional: Agregar constraint para blocked_by (referencia a la misma tabla)
-- Primero verificar si ya existe el constraint
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'fk_profiles_blocked_by' 
        AND table_name = 'profiles'
    ) THEN
        ALTER TABLE profiles 
        ADD CONSTRAINT fk_profiles_blocked_by 
        FOREIGN KEY (blocked_by) REFERENCES profiles(id);
    END IF;
END $$;

-- 8. Actualizar usuarios que ya tienen role 'blocked' para que tengan is_blocked = true
UPDATE profiles 
SET is_blocked = TRUE, 
    blocked_at = NOW()
